...
void findHome(){
  if(checkSensor(sensorPin0) || checkSensor(sensorPin1)){
    ...
    stepperPosition = moveStepperToPosition(HOMEPOS);
...
int moveStepperToPosition(int newPos){
  if(stepperPosition < newPos){
    digitalWrite(dirpin, LOW);
  } else {
    digitalWrite(dirpin, HIGH);
  }
  int steps = (abs)(stepperPosition - newPos);
  #ifdef SERIALCOMMANDDEBUG
    SERIALCOMMAND.print("Stepper moving to: "); 
    SERIALCOMMAND.print(newPos); 
    SERIALCOMMAND.print(" in ");
    SERIALCOMMAND.print(steps);
